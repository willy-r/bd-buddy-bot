...
import { formatBirthdayMessage, formatBirthdayLine } from '../../src/utils/date';
...
describe('formatBirthdayMessage', () => {
  ...
  it('returns isToday=true when birthday is today', () => {
    ...
    const result = formatBirthdayMessage({ birthdate: new Date(2000, 5, 15), show_age: false, age: 23, user_id: 'u1', guild_id: 'g1', username: 'u', guild_name: 'g' });
    expect(result.isToday).toBe(true);
  });

  it('includes age in today message when show_age is true', () => {
    vi.setSystemTime(new Date(2024, 5, 15));
    const result = formatBirthdayMessage({ birthdate: new Date(2000, 5, 15), show_age: true, age: 23, user_id: 'u1', guild_id: 'g1', username: 'u', guild_name: 'g' });
    expect(result.isToday).toBe(true);
  ...
  it('embeds months and days in future message', () => {
    ...
    const result = formatBirthdayMessage({ birthdate: new Date(2000, 2, 15), show_age: false, age: 23, user_id: 'u1', guild_id: 'g1', username: 'u', guild_name: 'g' }); // Mar 15
...
describe('formatBirthdayLine', () => {
  ...
  it('includes age+1 and user mention when show_age is true', () => {
    const result = formatBirthdayLine({ birthdate: new Date(2000, 5, 15), show_age: true, age: 23, user_id: 'u1', guild_id: 'g1', username: 'u', guild_name: 'g' });
    expect(result).toContain('<@u1>');
    expect(result).toContain('24');
  });

  it('omits age and uses 🎉 emoji when show_age is false', () => {
    const result = formatBirthdayLine({ birthdate: new Date(2000, 5, 15), show_age: false, age: 23, user_id: 'u1', guild_id: 'g1', username: 'u', guild_name: 'g' });
